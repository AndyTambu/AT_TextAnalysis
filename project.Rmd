...
```{r}
tibble_tweets$full_text <- gsub("RT", "", tibble_tweets$full_text) # remove RT
tibble_tweets$full_text <- gsub("http\\S+", "", tibble_tweets$full_text) # remove URLs
tibble_tweets$full_text <- gsub("@\\S+", "", tibble_tweets$full_text) # remove mentions
tibble_tweets$full_text <- gsub("[^[:alnum:]#\\.]+", " ", tibble_tweets$full_text) # remove punctuation
tibble_tweets$full_text <- tolower(tibble_tweets$full_text) # convert to lowercase


tibble_tweets$full_text %>% head()
```
```{r}
...
The first very obvipoous analysis that we can do is to make a words counting 
...
corpus <- Corpus(VectorSource(tibble_tweets$full_text))
